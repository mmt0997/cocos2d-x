                          ...
                          unsigned int step, float verticalSkirtSize)
{
    ...
    cocos2d::Vec3 min(FLT_MAX, FLT_MAX, FLT_MAX);
    cocos2d::Vec3 max(-FLT_MAX, -FLT_MAX, -FLT_MAX);
    ...
    for (unsigned int z = z1; ; )
    {
        ...
        for (unsigned int x = x1; ; )
        {
            ...
            {
                cocos2d::Vec3 p(v[0], computeHeight(heights, width, x, z), v[2]);
                cocos2d::Vec3 w(cocos2d::Vec3(x>=step ? v[0]-stepXScaled : v[0], computeHeight(heights, width, x>=step ? x-step : x, z), v[2]), p);
                cocos2d::Vec3 e(cocos2d::Vec3(x<width-step ? v[0]+stepXScaled : v[0], computeHeight(heights, width, x<width-step ? x+step : x, z), v[2]), p);
                cocos2d::Vec3 s(cocos2d::Vec3(v[0], computeHeight(heights, width, x, z>=step ? z-step : z), z>=step ? v[2]-stepZScaled : v[2]), p);
                cocos2d::Vec3 n(cocos2d::Vec3(v[0], computeHeight(heights, width, x, z<height-step ? z+step : z), z<height-step ? v[2]+stepZScaled : v[2]), p);
                cocos2d::Vec3 normals[4];
                cocos2d::Vec3::cross(n, w, &normals[0]);
                cocos2d::Vec3::cross(w, s, &normals[1]);
                cocos2d::Vec3::cross(e, n, &normals[2]);
                cocos2d::Vec3::cross(s, e, &normals[3]);
                cocos2d::Vec3 normal = -(normals[0] + normals[1] + normals[2] + normals[3]);
                normal.normalize();
                v[3] = normal.x;
                v[4] = normal.y;
                v[5] = normal.z;
    ...
    cocos2d::Vec3 center(min + ((max - min) * 0.5f));
...
bool TerrainPatch::setLayer(int index, const char* texturePath,const cocos2d::Vec2& textureRepeat, const char* blendPath, int blendChannel)
{
...
unsigned int TerrainPatch::computeLOD(Camera* camera, const BoundingBox& worldBounds) 
{
   ...
   cocos2d::Vec2 min(FLT_MAX, FLT_MAX);
   cocos2d::Vec2 max(-FLT_MAX, -FLT_MAX);
    worldBounds.getCorners(corners);
    for (unsigned int i = 0; i < 8; ++i)
    {
        const cocos2d::Vec3& corner = corners[i];
        float x, y;
...
const cocos2d::Vec3& TerrainPatch::getAmbientColor() const
{
    Scene* scene = _terrain->_node ? _terrain->_node->getScene() : NULL;
